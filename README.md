...
```bash
git clone https://github.com/coconilu/local-db.git
cd local-db
```
...
电脑重启后是否需要再次执行，取决于 Docker daemon 是否自动启动：

- 如果 Docker daemon 自动启动，并且容器是 `restart: unless-stopped`，通常不需要手动执行。
- 如果 Docker daemon 没有自动启动，先启动 Docker Desktop 或 Docker Engine，再执行 `docker compose up -d`。
...
下面示例使用 `curl`。如果你在 Windows PowerShell 里遇到 `curl` alias 行为差异，可以把命令里的 `curl` 改成 `curl.exe`。
